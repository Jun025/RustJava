...
**채택 제안 ★3건**: `2026-09-04-upstream-sync-s6#p1` · `2026-09-04-upstream-sync-s7#p1` · ★`2026-09-04-upstream-sync-s8#p0`
**성격**: 결정 회차 · 코드(`.rs`) 변경 **0** · 문서 ★**6파일**(규범 2: `CLAUDE.md`·`docs/upstream-sync-approach.md` / 기록 4: `REPORT.md`·`STATE.md`·이 `.md`·`.json`)
★**[fix3 정정] 초판은 「제안 2건 · 문서 2파일」이었다** — 제안은 세 번째 판(`s8#p0`)이 도착해 **셋**이고,
「문서 2파일」은 **규범 문서만** 센 수라 이 PR 이 실제로 만지는 **6파일**과 어긋났다.
...
| ⑦ | `double_must_use` allow(PR #14) | ★**9곳 «전건» 삭제** | ok | ★**beta RED**(rc=101 · 진단 **6**건) | ok | ★★**없음 — CI 만**(② 와 같은 칸) |
...
| ★③ **9곳 전건** | 0 | ★**RED** — rc=**101** · `double_must_use` 진단 **6**건(전건 `jvm/src/jvm.rs`) |
...
- ★★**② 와 ⑦ 은 «다른 자산»이 아니라 «한 근인의 두 얼굴»이다 — «로컬 DoD 가 CI 매트릭스를 재현하지 않는다».**
  ...
  ⇒ ★**근인 하나를 고치면 둘 다 그물을 얻는다**(실측: DoD 에 `+beta` 를 넣자 ⑦이 로컬에서 RED — rc=101 · 진단 **6**건 — 으로 잡힌다).
...
★**새 red 0**(baseline **554/0/1** 동수) · ★**돌연변이는 전부 복구**했고 ★**`.rs` 변경은 0**이다.
★**[fix3 정정] 초판은 「워킹트리는 문서 2파일만」이라 적었다** — 그 회차의 워킹트리 기준으로는 참이었으나,
★**이 PR 이 착지시키는 것은 문서 «6파일»**(규범 2 + 기록 4)이다. ★**같은 문장을 `verification` 에선 고치고 여기엔 남겼던 것**을 이제 닫는다.
