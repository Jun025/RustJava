...
★**두 축으로 갈라 읽어야 한다** — ⒜**편집 영역**은 «전부» 겹치고 ⒝**전달된 값**은 «3 중 1» 이다.
★초판은 이것을 「전부」로 뭉쳤고, ★**그 낱말이 카드를 `S` 로 과소 산정하게 만들었다**(둘은 같은 뿌리다).
...
⇒ 그래서 **새 카드**를 냈다(`proposals[0]` · effort ★**M**) — 원 제안은 `adoptedProposals` 로 처분하고,
**남은 절반만** 정확한 범위로 다시 세운다. ★**그렇게 하지 않으면 카드가 사라지면서 잔여도 함께 사라진다.**

## ★카드를 `S` → `M` 으로 올렸다 — 두 선택지를 «재고» 골랐다

★**초판의 `S` 는 «편집 영역이 전부 겹친다»를 «남은 일도 작다»로 읽은 결과**였다. 재보니 반대다 —
★**잔여도 #67 과 «같은 3층»을 건넌다**: 중간층 `jvm-bytecode/src/error.rs:5` 가 **`InvalidClassFile(&'static str)`** 이고
경계 `jvm/src/jvm.rs` 의 `exception(&str)` 이 **평범한 문자열**을 받으므로, 「argument_index 4 …」는
`src/runtime.rs:189`·`test-utils/src/lib.rs:334` 에서 **포맷**되어야 한다.

| 선택지 | 생성 자리 | 값 매치(`classfile/tests/test.rs`) | 층 | 판정 |
|---|---|---|---|---|
| ⒝ `InvalidFormat` 페이로드를 **넓힌다** | `validation.rs` **14** + `class.rs` **3** = **17곳** | **11곳 전건** | 3 | 편집 ≈30+ |
| ★⒜ **새 variant** | ★**1곳**(그 규칙만) | ★**0곳 강제** | 3 | 편집 ≈6 |

⇒ ★**⒜ 를 골랐다**(편집이 1/5 이고 기존 단정 11 을 건드리지 않는다). ★**그래도 `M` 이다** — 크기를 정하는 것은
**편집 수가 아니라 «건너는 층»** 이고, 잔여는 **5파일 / 4크레이트**(`classfile`·`jvm-bytecode`·`RustJava`·`test-utils`)를 건넌다.
★**층은 3인데 크레이트는 4다 — 둘을 같은 수로 적지 마라**: 경계 층 하나가 `RustJava`(`src/runtime.rs`)와
`test-utils`(`test-utils/src/lib.rs`) **두 크레이트**에 걸친다(`cargo metadata --no-deps` 귀속). 이 사실은 `M` 의 근거를 **강화한다**.
★★**원 제안이 «전체»를 `M` 으로 값했는데 «미전달된 절반»을 `S` 로 값하면 순서가 뒤집힌다** — 그 역전을 되돌린 것이다.

★**⒜ 의 대가**(카드 `tradeoff` 에도 적었다): ★**거부 오류가 «두 갈래»가 된다** — `InvalidFormat(&'static str)` 과
구조화된 variant 가 나란히 서고, 「왜 이건 구조화고 저건 맨 문자열인가」를 **이후 규칙마다** 답해야 한다.

## ★제안이 «적지 않은» 설계 제약 하나 — 후속이 다시 발견하지 않도록

`ClassFileError` 는 ★**`Copy` 를 derive 한다**. 인덱스를 담겠다고 `String` 을 넣으면 **`Copy` 가 깨지고**
그것은 그 소비자에게 전파된다 — ★**파일 4 · 크레이트 «2»**(`classfile` 3 + `jvm-bytecode` 1 · ★초판의 「네 크레이트」는 명사가 틀렸다 · 수는 맞다 · 루트 2건은 **주석**)(`classfile/tests/test.rs` **13** · `validation.rs` **8** · `class.rs` **7** ·
...
- ★**`S` 에서 `M` 으로 올린 대가 — 카드가 뒤로 밀린다.** 실측이 올린 값이라 되돌리지 않지만,
  `S` 였다면 먼저 집혔을 자리에서 내려가고 그만큼 열린 카드가 는다. 그 손실을 여기 적는다.
