...
# ★**포팅: `qts` 는 «불가» · `wie` 는 «상수 복사로는 불가 · 파서 4축 수정 후 가능»**
...
## 0. ★★[게이트² F1 정정] 이 PR 은 «스택»이다 — 착지 순서와 예정된 충돌

★**초판은 이 사실을 «한 줄도» 적지 않았다**(`base main` 뿐). 실측이다:

```
PR #28 : 61d5bf3 → 204f98b [#27 의 커밋] → ed04620
PR #27 : 61d5bf3 → 204f98b              → e9e2a8e1
git merge-base --is-ancestor 204f98b  <#28 head>  → rc=0   ★#28 은 #27 의 커밋을 «업고» 있다(스택)
git merge-base --is-ancestor e9e2a8e1 ed046208    → rc=1   ★#28 은 #27 의 «옛» 상태를 업고 있다
git merge-tree --write-tree e9e2a8e1 ed046208     → rc=1   CONFLICT: STATE.md
```

⇒ ⑴★**스택 사실**: `204f98b`(=#27 의 구현 커밋)이 이 브랜치의 **조상**이다.
⑵★**착지 순서 = `#27` → `#28`.** ★단 **base 는 `main`** 이라 #27 의 브랜치가 지워져도 이 PR 이 «닫히지»는 않는다
(계약 5 의 `ripple` 사고는 자식의 base 가 «부모 브랜치»였던 형상이다 — **다른 형상**이다).
⑶★★**#27 착지 «후» `STATE.md` 충돌이 «예정»돼 있고, 그때 그 항목의 재작성이 필요하다.**

★**초판 작성 시점에 `MERGEABLE` 로 보였던 이유**: GitHub 이 ★**«#27 이전» main** 과 대조했기 때문이다.
★★**[갱신 · 이 정정 회차 실측] 그 «예정»은 «실현»됐다** — #27 이 `ba4bb417` 로 **착지했고**,
워밍 후 재조회에서 이 PR 은 ★**`CONFLICTING` / `DIRTY`**, 충돌 파일은 ★**`STATE.md` «1건»**이다
(`git merge-tree --write-tree --name-only <#28 head> origin/main` → rc=1 · `STATE.md`).
⇒ ★**그 1건은 게이트③ 계약 2-c⒜ 의 «원장 파일 한정» 승인 범위 «안»이고 코드 파일 충돌은 0** 이다.
★★**이 회차는 그것을 «해소하지 않는다»** — 해소는 게이트③의 몫이고, ★**충돌 집합은 «그때의 값»이라 그 회차가 착수 시점에 다시 재야 한다**(2-c⒜⑴).
...
### ⒜ `wie` — RustJava 파서 «규칙»을 적용했을 때(★**손으로** — 도구를 그대로 돌린 값이 아니다)

★★**[게이트² F2 정정] 초판의 「«그대로» 적용」은 «부정확»했다.** 아래 표는 ★**후행 주석을 손으로 벗기고** 잰 값이다.
★**분석값 «1» 은 옳다 — 틀린 것은 «그대로» 라는 말이다.** 도구를 실제로 돌리면 다른 수가 나온다(→ §4⒜ 넷째 축).
...
★★**`wie` 는 «포팅 불가»가 «아니다» — «상수 복사로는 불가 · 파서 «네 군데» 수정 후 가능»이다**(F3 정정):
...
⑷★★**DoD 줄의 «후행 주석» 절삭**(F2) — 아래 §4⒜ 의 실행 출력이 그 증거다.

★★**[게이트² F2 · 실행으로 확인] 상수 «1줄»(`DOD_FILE` → `AGENTS.md`)만 바꿔 `wie` 에 돌렸다**
(경로·제목은 `wie` 도 같아 치환이 필요 없었다) — ★**rc=1 · 축 A 대칭차 «7»**(CI 에만 **3** · DoD 에만 **4**) · 축 B 는 `beta` **1**.
⇒ ★**`wie` 에서 그 도구는 «돈다»**(`qts` 는 `sys.exit`) — 그래서 「포팅 불가」는 `qts` 에만 참이다.

### ⒝ `qts` = ★**⒞ 조건부 «필요»**

★★**RustJava 검사기는 ★`qts` 에서 «포팅 불가»다 — 구조가 다르다**(★`wie` 는 «돈다» — 위 ⒜ 참조):
...
- **wie 전용 축 ★«넷»**: ⑴조건부 step 을 «게이트/설정»으로 가를 술어 ⑵env 접두 정규화 ⑶**다중 워크플로 스캔**
  ⑷★★**DoD 줄의 «후행 주석» 절삭** — `wie` 의 four gates **4줄이 전부** 후행 주석을 달고 있는데
  (`cargo fmt --all -- --check   # rust.yml: formatting gate`) `parse_dod → norm()` 은 **공백만 접고 주석을 벗기지 않는다**.
  ★**RustJava 자신의 DoD 에는 주석이 «없어서» 그 파서가 이 축을 만난 적이 없다.**
  ★**안 늘리면 다음 워커가 «거짓 대칭차 7건»을 만난 뒤 회차 «중간»에 이 축을 발견한다.**
- **qts 전용 축**: **Makefile 타깃 전개** · 축 B 부재 처리 · action 기반 검사를 «못 본다»고 **명시 출력**.
- ★**T3 를 T1·T2 «앞»에 두지 마라** — 락은 «어긋남을 막는» 것이지 «이미 난 어긋남을 고치는» 것이 아니다.
- ★★**[게이트² F4] «언제 락이 필요해지는가»를 «잴 명령»과 «오늘의 값»으로 적어라** — 부모 회차(#27)가 같은 자리에
  그 형태를 박았다. ★**T1·T2 가 «오늘의 어긋남»을 닫아 버리므로, 명령이 없으면 ⒞(조건부)를 기계로 다시 물을 수 없다.**
  ⇒ **T3 합격선에 이 한 줄을 넣어라**(repo 마다 `<slug>`·`<workflow>`·`<로컬이 치는 명령>` 만 바꾼다):
  ```sh
  # 「로컬 DoD 가 «안 치는» step 이 CI 실패에 등장한 run」의 수 — 0 이 아니면 락이 값한다
  gh run list --repo <slug> --workflow=<workflow> --status=failure --limit 200 \
    --json databaseId --jq '.[].databaseId' \
  | while read -r id; do
      gh api "repos/<slug>/actions/runs/$id/jobs" \
        --jq '[.jobs[]|select(.conclusion=="failure")]|map(.steps//[]|map(select(.conclusion=="failure"))|map(.name))|flatten|unique|join(";")'
    done | /usr/bin/grep -c '<로컬이 안 치는 명령>'
  ```
  ★**오늘의 값**: `wie` = **9**(beta 셀 전용 clippy · `cargo +beta` 미보유) · `qts` = **38**(`ruff format --check`).
  ★**T1·T2 착지 «후»에 다시 재라 — 그 값이 «0 에 가까워지면» T3 의 실익이 준다.**

## 6. 경계

★**형제 repo 쓰기 0** — `git` 명령 0 · PR 0 · 파일 수정 0(전부 `gh api … /contents` 읽기).
★`.rs` **0줄** · ★★**이 회차 커밋 기준** 이 repo 의 **검사기·`rust.yml`·DoD 블록 무접촉** ·
★★**[게이트² F1-⒝ 정정] PR 파일 목록에는 `scripts/check-dod-ci-parity.py` 가 «5+» 로 보인다 — 그 5줄은 «#27 의 것»이다**
(이 PR 이 #27 «위에» 얹혀 있기 때문이다 · §0). ★**이 회차 커밋은 결백하다**:
`git diff 204f98b..ed046208 -- '*.rs' scripts/ .github/ CLAUDE.md` → ★**0**.
⇒ ★**게이트③ 집행자가 PR 파일 목록만 보면 «회신과 반대되는 것»을 읽는다** — 그래서 문면을 «이 회차 커밋»으로 한정한다.
