...
「모르는 도구 = 즉시 red」는 **새 게이트**라 이 회차 범위 밖이다. ⑵블록 스칼라 평탄화(⒞의 env 보존)는 **하지 않았다** — 이 repo 엔 그 형상이 없고(M3 이 red 로 운다), wie 가 이미 푼 축이다.
★★**[2026-09-08 추후] ⑴ 의 「이 회차 범위 밖」은 «닫혔다» — 아래 ⑺ 가 그 결정이고 «집행»이다.**

#### ⑺ [2026-09-08 결정+집행] 「모르는 도구」는 ★**red 다 — 단 «등록 한 줄»로 지나간다**(⒞ 조건부)

채택 제안 `2026-09-08-parity-axis-a-tool-name#p0` 에 대한 **결정**이다. 세 선택지 중 ★**⒞ 조건부**를 골랐다.

> ★**결정: `CHECK_TOOLS` 에도 `SETUP_TOOLS` 에도 «없는» 도구를 부르는 `run:` step 은 «FAIL» 이다.**
> ★**등록은 한 줄** — 진짜 검사면 `CHECK_TOOLS`(+ DoD 블록), 셋업이면 `SETUP_TOOLS` 에 «왜»와 함께.

**★먼저 «장래 위험»이 아니었다 — 오늘 이미 1건 있었다**

발권 소견은 「목록 밖이 **0** 이면 ⒞ 가 값한다(비용 0)」였다. ★**재보니 0 이 아니라 «1»이다**:

| 축 | 수 |
|---|---|
| `rust.yml` 의 `- run:` step 총계 | **7** |
| 축 A(`CHECK_TOOLS` = `cargo`·`python3`) | **6** |
| ★**목록 «밖»** | ★**1 — `git`**(`<셸 블록> git config --global core.autocrlf false` · windows 줄끝 셋업) |

⇒ ★★**이 수가 설계를 «고쳤다».** 「목록 밖 = 즉시 red」(⒜의 소박한 형태)로 갔으면 ★**착지 첫날부터 «옳은 워크플로»가 red** 다.
★**「목록 밖」은 «한 가지»가 아니라 «두 가지»였다** — ⑴DoD 가 몰라야 할 **셋업**(`git`) ⑵DoD 가 배웠어야 할 **미등록 검사**(`npm`).
그 둘을 가르는 것이 `SETUP_TOOLS` 이고, ★**그래서 ⒞ 는 «⒜의 완화»가 아니라 «⒜가 성립하는 유일한 형태»다.**

**★버린 이유 — 둘 다 적는다**

- ⒜**무조건 red** — ★**버렸다**: 위 실측대로 **오늘 당장 위양성 1건**이고, 등록 경로 없이 red 만 올리면
  새 검사를 «정당하게» 들이는 회차가 막혀 ★**우회(도구를 `sh -c` 로 감싸기 등)를 만든다**. 우회는 이 저장소가 반복해 규탄한 형태다.
- ⒝**«찍기» 그대로** — ★**버렸다**: 가장 싸지만 ★**구멍이 «조용하다»**. `npm test` 한 줄이 들어와도 rc=0 이라
  **아무도 안 읽으면 아무 일도 일어나지 않는다.** ★이 리니지가 다섯 번 낡은 방식이 정확히 「사람이 읽어야 효력이 있는 문서」다.
  ⇒ ★**「알고 두는 것」으로 만들려면 «기계가 말해야» 한다** — 출력 한 줄은 그 조건을 만족하지 못한다.

**★개악 대조 12종** (측정 트리 = `git clone -s` 스크래치 · 핀 `f133cd3044b9003bbabb98f9164b8a7184a4ea9a`)

| # | 편집 | 옛(=`f133cd30`) | ★새 | 읽는 법 |
|---|---|---|---|---|
| **N1** | ★**지정 개악** — `- run: npm test` 한 줄 | ★**green(rc=0)** | ★**red(rc=1)** | ★**이 회차의 유일한 성공 기준.** `★ CI 가 «모르는 도구»를 부른다 — 도구=npm` |
| N1' | 원복 | green | ★**green** | 되돌리면 통과 |
| N2 | `- run: make lint` | green | ★**red** | `npm` 전용 술어가 아니다 |
| N3 | `npm test` 를 `if:` 아래로 숨김 | green | ★**red** | ★선행 회차의 「`if:` 는 소속을 정하지 않는다」가 **여기서도** 산다 |
| **N4** | ★**등록 경로 ⑴** — `npm` 을 `SETUP_TOOLS` 에 한 줄 | — | ★**green** | ★**등록 경로가 «실재»한다**(셋업 갈래) |
| **N5** | ★**등록 경로 ⑵** — `npm` 을 `CHECK_TOOLS` 에 + DoD 줄 | — | ★**green**(명령 **7**개) | ★**검사 갈래** — 축 A 가 실제로 7로 늘어난다 |
| N6 | ★**DoD 쪽 거울** — DoD 블록에만 `npm test` | green | ★**red** | ★**술어를 양쪽에 «같게» 걸었다**(선행 회차 규율 승계) |
| **M2** | ★**반대 개악** — `cargo test --all` step 삭제 | red | ★**red** | ★**기존 축 A 가 여전히 산다** |
| M1b | 선행 회차의 지정 개악(`if:` + DoD 삭제) | red | ★**red** | ★**선행 술어 무손상** |
| B1~B3 | 무해 — 빈 줄 · step `name:` · step 순서 | green | ★**green** | ★**위양성 0** |

**★선행 술어를 «다시 만들지» 않았다 — `git diff` 로**

`git diff --numstat f133cd3044b9003bbabb98f9164b8a7184a4ea9a -- scripts/check-dod-ci-parity.py` → ★**`33  0`**
= **추가 33줄 · 삭제 0줄**. ⇒ `parse_ci`·`tool_of`·`CHECK_TOOLS`·축 A/B 비교 로직은 **한 줄도 안 건드렸다**.

**★천장 — 알고 둔다**

⑴★**`sh -c 'npm test'` 처럼 셸을 «한 겹» 씌우면 도구가 `sh` 로 읽힌다** — `sh` 는 두 목록에 없으므로 ★**red 가 난다**(우회가 아니라 걸린다).
★**주장이 아니라 실측이다**(N7): `- run: sh -c 'npm test'` → **rc=1** · `★ CI 가 «모르는 도구»를 부른다 — 도구=sh: sh -c 'npm test'`.
그러나 `sh` 를 `SETUP_TOOLS` 에 등록해 버리면 그 겹 아래는 다시 안 보인다 — ★**등록은 «한 줄»이지만 «공짜»는 아니고, 그 줄이 리뷰 대상이다.**
⑵**블록 스칼라 평탄화**는 여전히 하지 않았다(선행 회차 그대로) — 블록 스칼라의 도구는 **첫 줄**로 읽는다. 둘째 줄부터의 도구는 안 본다.
⑶★**새 워크플로 0 · 새 잡 0** — 판정은 기존 `dod_parity` 잡의 rc 에 얹혔다. **CI 를 막는 «다른» 게이트를 만들지 않았다.**
...
## 5-B. ★★★[2026-09-04 판정] 다음 동기는 «언제» — **격차 기반 `behind ≥ 20`**

채택 제안 `2026-09-04-sync-contract-stale-assets-decision#p0` 에 대한 **결정**이다.
★**S1~S8 이 behind 33 → 0 으로 캠페인을 닫았고, 「다음에 언제 받을지」가 비어 있었다.**

> ★**동기 회차는 `git rev-list --count origin/main..upstream/main` 이 «20 이상»일 때 연다.**
> ★**시간(격주·월간)으로 열지 않는다.** ★**그 아래에서는 열지 않는다** — 아래 실측이 그 이유다.

### ⒜ ★★「벌어진 뒤에 받으면 훨씬 비싸다」는 이 저장소 데이터가 «반증»한다

★★**[2026-09-04 정정 · 게이트②] 초판 표는 «맨 수» 8행이었다 — 이 파일의 상시 규칙(`§5` 「충돌 수를 적을 때는
base 를 반드시 병기한다」)을 «이 표가» 어겼고, 실제로 기준이 섞여 있었다**(S3·S6 은 델타 · S4 는 ★**계획서 «예측»** · S8 은 누적).
...
|---|---|---|---|---|---|
| S1 | 5 | **2** | 2 | `03438b0` · `62cf0c6a` | 설계(tracing) |
| S2 | 1 | **5** | +5 | S1 착지본 · `1f356ae` | 설계(charset) |
| S3 | 1 | ★**11** | +9 | S2 착지본 · `af4f6f8` | 설계(오류 분류) |
| S4 | 8 | ★**2** | ~~0(★**계획서 «예측»**)~~ | `3a59776`(복원 후) · `822504b` | 물량 |
| S5 | 6 | **3** | +3 | `8c1238b` · `3296139c` | 물량 |
| S6 | ★**11** | ★**1** | +0 | `a0b5d3c` · `c4665b0` | 물량 |
| S7 | 1 | **1** | +1 | `3e02f8c` · `95ebc5c` | 물량+ |
| S8 | ★**12** | **8** | +8 | `3fb08a8` · `ba5797b` | 개명 스윕 |

★★**정정 셋을 «숨기지 않는다»**:
- ★**S4 = `0` 은 «실측이 아니라 계획서 예측»이었다**(`reports/…-s4.done.md`: 예측 **0** / 복원 «전» 실측 **20** / 복원 «후» 실측 **2**).
  §5 제안표의 그 칸만 **한 번도 정정되지 않았고**, ★**같은 파일의 정정 블록은 이미 `S4 20 → 2` 를 적어 두고 있었다.**
  ★**하필 그 칸이 논증의 «최강 데이터»(「8커밋인데 충돌 0」)였다** ⇒ **실측 `2` 로 고친다.**
- ★**S6 = `0` 은 «델타»다 — 누적은 `1`**(`string.rs`). ★**§5 가 그 오독을 «실사고»로 이름 붙인 바로 그 자리다.**
- ★**S3 = `+9` 는 델타 · 누적은 `11`**(착지 커밋 제목 「충돌 11」).

★★**세 열을 «다 계산해» 싣는다 — 부호가 안 흔들린다는 것이 요지다**(내가 다시 계산했다):
...
★★**세 열 모두 r 의 부호가 «음»이고 「많은4 < 적은4」가 유지된다** ⇒ ★**판정은 흔들리지 않는다.**
★★**단 r 을 «정밀 추정치»로 읽지 마라 — n=8 이고 회차 경계가 «컷 분할»로 정해진 비무작위 표본이다.**
★**이 수는 «비례하지 않는다»는 «부호 판정»으로만 쓴다**(초판이 그 열로 「우상향」을 읽은 것과 **같은 방법**이라
방향만 반대다 — 그래서 강하게 읽지 않는다).

★**정본 기준(C)으로도**: 커밋 많은 4회차(**37커밋**)가 충돌 **14** · 적은 4회차(**8커밋**)가 충돌 **19**.
⇒ ★★**충돌을 만든 것은 «몇 개 왔는가»가 아니라 «무엇이 왔는가»다** — 우리 포크 지점
(tracing span · charset 라우팅 · 오류 분류)을 upstream 이 건드릴 때, 그리고 개명 스윕 같은 **사건**이 올 때.

★**누적 충돌도 «포화»한다**(§5 첫 표): 7커밋 **16** → 10커밋 17 → 21커밋 17 → 32커밋 17 → 33커밋 **19**.
⇒ ★**7커밋 뒤로 26커밋을 더 벌려도 충돌이 «+3»** 이다. 「격차가 크면 비용이 는다」의 흔한 기전이 **여기엔 없다**.

### ⒝ ⇒ 그래서 «작게 자주»가 총비용을 낮추지 «않는다»

★**회차 오버헤드는 «고정»이다** — 회차당 티켓 **3건**(구현 · 게이트② · 게이트③).
게이트② 사이클 실측: **8회차 중 7회가 1사이클**(S4 만 3).
⇒ ★**충돌 총량이 포화하는데 회차 수만 늘면 «총비용이 는다».** 시간 기반(⒜)을 기각한 이유가 이것이다.
★**그리고 upstream 이 느리고 버스티하다**(아래 ⒞) — 시간 축은 ★**behind 0 인데 회차를 여는 «빈 회차»** 를 만든다.

★**「8회차가 들었다」를 «격차 33 의 비용»으로 읽지 마라** — 회차 경계는 §5 의 **컷 분할**이 정했고,
추가 비용의 큰 덩어리는 ★**스쿼시로 계보가 접힌 것**이었다(별 리니지 `…-squash-defeats-convergence`).
그것은 격차가 아니라 **머지 방식**의 비용이고, `merge_strategy: merge` 로 이미 닫혔다.

### ⒞ upstream 속도 실측(12개월 · `upstream/main`)

**144 커밋 / 51주** — 주당 **평균 2.81 · 중앙 1.0 · p90 7** · ★**커밋이 «0»인 주가 19주(37%)**.
누적 도달 시간(연속 창 실측): behind **5** 중앙 **8일** · **10** 중앙 **21일** · **20** 중앙 ★**44일** · **33** 중앙 83일.
⇒ ★**`behind ≥ 20` 은 실질 «약 6주 · 연 6~8회»** 다.
★★**「44일」을 «상수»로 인용하지 마라** — 게이트② 재측은 **41일**이었고 차이는 **창 정의**(어느 12개월을 쓰나)에서 온다(±3일).
★**결론 영향은 0**(어느 값이든 「약 6주」다) — 그러나 인용할 때는 **그 창을 함께 적어라**.

### ⒟ 왜 «20» 인가 — 위아래를 둘 다 적는다

- **아래로 내리지 않는 이유**: 충돌이 **7커밋에서 포화**하므로 그보다 자주 받아도 충돌 총량이 안 준다.
  더 자주 받으면 회차 수(= 티켓 3건씩)만 는다.
- **위로 올리지 않는 이유**: ★**포화는 «7~33 구간»에서만 측정됐다 — 33 너머는 «미측정»** 이다
  (§5 첫 표의 상한이 33 이다). ★**미측정 구역으로 걸어 들어가지 않는다.**
  ★★**[2026-09-04 정정 · 게이트②] 초판은 여기에 「33 은 실제로 아팠던 값이다」를 함께 적었다 — «지웠다».**
  ★**그것은 바로 위 ⒝ 가 «금지한» 독법이다**(「8회차가 들었다」를 격차의 비용으로 읽는 것) ⇒ **같은 절 안의 자기모순**이었다.
  ★**「미측정」 논거는 혼자 선다** — 아픔을 끌어올 필요가 없다.
- ★**그리고 «늦게 알수록» 우리 자산 낡음이 오래 방치된다** — S8 개명 스윕이 `rust.yml` 의 크레이트 이름과
  경로 4곳을 낡게 만들었고, 그 여파가 **문서 정정 5회차**로 이어졌다(§4 리니지).

★★**[2026-09-04 명시 · 게이트②] 「20」은 «밴드 안에서 «고른» 수»다 — 데이터가 그 한 점을 지목하지 않는다.**
★**지어낸 수가 «아니다»**: **밴드**가 실측이다(하한 = 포화 시작 **7** · 상한 = 측정 상한 **33**).
★**그리고 그 밴드 안 누적이 `16 → 17 → 17 → 17 → 19` 로 «평평»해 어디를 골라도 데이터는 같은 말을 한다.**
⇒ ★**「20 을 골랐다」** — 근거는 «도달 중앙 ≈6주가 회차 오버헤드(티켓 3건)와 균형」이라는 ★**판단**이고,
★**⒡ 가 3회차 뒤 재측정을 걸어 그 판단을 검증한다.** ★**밝히는 것이 논증을 약화시키지 않는다** — ⒡ 가 그 전제 위의 장치다.

### ⒠ ★누가 방아쇠를 당기는가 — **기계가 «재고», 사람(총괄)이 «발권한다»**

```sh
git fetch -q upstream && git rev-list --count origin/main..upstream/main   # ★오늘의 값 = 0
```
- ★**재는 자리(권고) = 이 repo 의 «예약 워크플로»** — 선례가 있다(`rust-audit.yaml` 이 이미 `schedule: cron`).
  upstream 을 **read-only fetch** 해 그 수를 찍고 임계 초과 시 **눈에 보이게** 알린다. ★**upstream 발신 0**(fetch 는 읽기다).
...
★**⑵의 중앙이 «5 이상»으로 오르면 임계를 내려라**(포화 가정이 깨진 것) ·
★**⑴이 «14일 이하»로 짧아지면 임계를 올려라**(upstream 이 빨라진 것 — 지금은 중앙 44일).
