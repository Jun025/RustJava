...
- ★**오탐을 세었다**: 최근 200커밋·머지 83건 소급 → 초판 **8건/19정의** · ★**확장본 10건/45정의**(게이트② 실측) · **4.4초/머지** ⇒ PR 당 내부 머지 0~2건이라 **실비용 ≈ 0~9초**. ★늘어난 것에 **진짜 사고 1건이 들어 있다**.
- ★**trailer 이름은 «출력에서 복사»해야 한다** — `method fieldref` 는 통하고 `Pool.fieldref` 는 **안 통한다**. ★초판 docstring 의 유일한 예시가 **안 통하는 쪽**이었다(게이트② 지적) ⇒ 예시를 고치고 **「글자 그대로 같아야 한다」를 계약에 박았다**. ※당시 전 이력 trailer **0건**이라 깨진 등재는 없다. ★CI 는 `origin/main..HEAD` 만 보므로 **과거 8건이 main 을 red 로 만들지 않는다**.
- ★**잃는 것**: ★**이름은 남고 본문이 비어진 경우는 못 잡는다** · 리네임·이동을 소실과 **구별 못 한다** · ★**trailer 는 우회로다**(사유 필수 + 커밋에 남는 것이 유일한 브레이크 — 더 조이지 않은 것이 판단이다) · 머지마다 4.4초가 붙고 **DoD 블록에도 들어가** 로컬에도 붙는다.
