...
impl InputStreamReader {
    ...
    async fn init_with_charset(
        ...
        let charset_name = JavaLangString::to_rust_string(jvm, &charset).await?;
        let charset_name = Charset::resolve(jvm, &charset_name).await?.canonical_name();

        Self::init_fields(jvm, this, r#in, charset_name).await
    }

    async fn init_fields(jvm: &Jvm, mut this: ClassInstanceRef<Self>, r#in: ClassInstanceRef<InputStream>, charset_name: &str) -> Result<()> {
    ...
    async fn read(
        ...
        while write_buf_size < length {
            ...
            let (read, wrote) = decoder.decode_to_utf16(&read_buf_data[..decode_length], &mut decoded, end_of_input);
