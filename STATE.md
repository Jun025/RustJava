...
- [rustjava-upstream-sync-s4] upstream 컷 `3296139`(#184 GlobalRef · CLI classpath · CDC text) 머지 —
  ...
  (트리 무변경 실측) — 그것이 ★**충돌 20 → 2**를 만들었다. **PR 대기 — 게이트③ 미착지.**
  ★★**계획서의 「S4 새 충돌 0」 예측은 틀렸다 — 실측 2건**이고, `thread.rs` 는 **S1·S3 에 이어 세 번째**다.
  ★**upstream 회귀 1건을 들여왔다**: `test_timer_periodic` 이 컷 `3296139` 에서 flaky 가 됐다
  (★**순정 upstream 에서 재현** — 우리 해소 탓이 아니다). 여백만 넓혔고 단정은 그대로다.
...
### ①(최우선) upstream 동기화 — ★**S4 착지 대기(2026-08-27)**. 정본 = `docs/upstream-sync-approach.md`

★**S4(`3296139` · 물량 회차)까지 머지 완료 · PR 대기 중이다. 다음은 S5(`c4665b0` · Java 1.2 API 확장).**
...
⇒ 첫 조치 `git merge -s ours --no-ff 822504b`(트리 무변경 **0줄** 실측) → `merge-base` **`822504b`** 복원.
★**충돌 20 → 2**(`java/lang/thread.rs` · `jvm/src/jvm.rs`).
green 전건 rc=0 · `cargo test --all` **261 passed / 0 failed / 1 ignored**(S3 216 → +45).

★★**이 회차가 upstream 회귀를 하나 들여왔다 — 숨기지 않는다.** `test_timer_periodic`(우리가 만든 테스트가
아니다)이 컷 `3296139` 에서 **flaky** 가 됐다. 3트리 대조 실측:
`origin/main` **10/10 pass** ↔ ★**순정 upstream `3296139` 도 `RUNCOUNT` 2~6 으로 3/8 실패** ↔ 우리 머지 2~3.
⇒ ★**우리 해소 탓이 아니다**(수동 span 제거 프로브에서도 같은 비율 — 그 축이 아님이 실측됐다).
근인 후보 = `e557673`(GlobalRef)이 GC 에 전역참조 스캔을 추가해 TimerThread 1회전이 **~110~150ms** 로 늘어난 것.
처분 = ★**여백만 넓혔다**(`sleep 500ms → 2000ms`) · **`run_count > 2` 단정은 불변** · `#[ignore]` 0 · 테스트 삭제 0.
upstream 도 같은 자리를 두 번 넓혔다(`895d67d`·`ad8b477`) ⇒ 선례에 맞는 처분이다.
★**남은 것은 «성능 회귀 그 자체»이고 그것은 별 축이다** — upstream 발신이 필요하므로 이 리니지 밖이다.

★**「예측은 하한」이 이제 3회 연속 실측됐다**: S3 +9 예측 ↔ 11 · S4 **0 예측 ↔ 2**.
★**`thread.rs` 는 S1·S3·S4 «세 회차 연속» 충돌**한다 — upstream 이 `ThreadStartProxy::call` 을 반복 재작성하기 때문이다.
⇒ **S5~S7 도 그 파일은 「또 충돌한다」를 기본값으로 잡아라.** 해소 전략은 불변이다:
