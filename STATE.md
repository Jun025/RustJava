...
- [rustjava-worklog-mandate-decision-and-local-gate] ★**워크로그 작성을 DoD «의무»로 결정 + 형식 잠금을
  ...
  대가 = **+0.10s**(3명령 warm 합계 60.2s 대비 **+0.17%**). **PR 대기 — 게이트③ 미착지.**
- [rustjava-upstream-sync-remeasure-s5-s7-and-lock-restore-basis] ★**PR #18 착지 뒤 기준 S5~S7 재측정 +
  「충돌 수에 base 병기」를 §5 상시 규칙으로 채택**(PR **#19** 대기). 같은 컷 `ba5797b` 가
  **19 · 112 · 4** 로 갈린다 · 새 충돌 **S5 +3 · S6 +0 · S7 +1** · ★**S8 이 필요하다**(개명 스윕).
